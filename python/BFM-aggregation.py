...
def iterate_forward(flt2d, method, push, psi, phi, mu, DUstar, V, dW, kernel, n, tau, sigma, theta1, theta2, m) -> float:
    flt2d.find_c_concave(psi, phi, tau)
    flt2d.find_c_concave(phi, psi, tau)


    phi_dW = phi + dW
    method.calculate_DUstar(DUstar, phi_dW, V, m, mass)
...
def iterate_backward(flt2d, method, push, psi, phi, mu, DUstar, V, dW, kernel, n, tau, sigma, theta1, theta2, m) -> float:
    flt2d.find_c_concave(psi, phi, tau)
...
mu = np.zeros((n,n))
mu[(np.abs(xx-0.2)<0.1) & (np.abs(yy-0.2)<0.1)] = 1
mu[(np.abs(xx-0.2)<0.1) & (np.abs(yy-0.8)<0.1)] = 1
mu[(np.abs(xx-0.8)<0.1) & (np.abs(yy-0.2)<0.1)] = 1
mu[(np.abs(xx-0.8)<0.1) & (np.abs(yy-0.8)<0.1)] = 1
mu *= mass / mu.mean()
...
dW     = np.zeros((n,n)).astype('float64')
...
for jj in range(1,21): # total number of outer iterations rho^0, rho^1, ..., rho^{jj}
    ...
    method.calculate_dW(dW, mu)
    dW *= 50

    pbar = tqdm.tqdm(range(1000))
    for i in pbar: # you may need samller numbero fiteration than this. 
        error_for = iterate_forward (flt2d, method, push, psi, phi, mu, DUstar, V, dW, kernel, n, tau, sigma, theta1, theta2, m)
        error_bac = iterate_backward(flt2d, method, push, psi, phi, mu, DUstar, V, dW, kernel, n, tau, sigma, theta1, theta2, m)

        error = min(error_for, error_bac)

        pbar.set_description(f'Iter: {jj} error: {error:0.2e}, {error_for:0.2e}, {error_bac:0.2e}')

        if error < 1e-4 * mass: # stopping condition
            break
